...
Grid with filters (filter by: month, status, batch, class)
...
Export can be filtered by month, batch, class, or status before downloading.
...
Fee amount for a student is managed from the Payments module — admin opens a student's payment record and updates the fee amount from there.
