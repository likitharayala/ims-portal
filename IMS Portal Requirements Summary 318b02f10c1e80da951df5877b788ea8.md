...
Grid layout
Grid columns: Name, Email, Phone, Class, School, Parent Name, Parent Phone, Joined Date
...
Sorting (by: name, join date, batch, class — ascending and descending)
...
Fee amount: required field — admin sets the monthly fee amount for the student at creation
...
Materials are visible to ALL students of the institute — no batch or class filtering.
...
Assessments are visible to ALL students of the institute — no batch or class filtering
Admin can edit an assessment at any status (draft, published, active, closed, evaluated)
Admin can soft-delete an assessment at any status — submissions are also hidden
Admin can duplicate an assessment — copy is created as a new draft with the same questions and settings
...
Payment export:
Admin can download all payment records as an Excel file (bulk export).
Export includes: student name, roll number, class, batch, month, amount, status, notes.
...
Notification message maximum length: 500 characters.
